...
- In this project, will link Databases and Python. 
...
I learned about how object-relational mapping is used for database scripting. I became familiar with using MySQLdb and SQLAlchemy to
query, create, edit, and delete tables in MySQL.

## Tests :heavy_check_mark:

* [tests](./tests): Folder of SQL and Python scripts for setting up test tables
for all files. Provided by ALX Africa.
...
- Your script should take 3 arguments: `mysql username`, `mysql password` and `database name`
- You must use the module SQL`Alchemy
- You must import `State` and `Base` from `model_state - from model_state import Base`, `State`
- Your script should connect to a MySQL server running on `localhost` at port `3306`
